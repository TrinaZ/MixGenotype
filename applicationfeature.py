...
for q in range(0,1):
    ...
    class Variation:
        ...
        def SetFeature(self, AbnormalRead, NormalRead, IncompleteRead, FullyRead, SingleRead, SplitRead, FsamRead,
            ...
            self.Result = Result
    f = open('F:/importantfilecopy/tandomrepeat/2019.121test/5.sam', 'r')
    line = f.readline()
    line = f.readline()
    for line in f:
    ...
    VariationList = []
    type = []
    f2 = open('F:/importantfilecopy/tandomrepeat/2019.121test/v8.txt', 'r')
    for line1 in f2:
        line1 = line1.strip().split("\t")
        start.append(line1[1])
        pos = line1[7]
    ...
    for (elem_start,elem_end) in zip(list(start),list(end)):
        ...
        spread1left = []
        spread2left = []
        f = open('F:/importantfilecopy/tandomrepeat/2019.121test/5.f.sam', 'r')
        line = f.readline()
        line = f.readline()
        # line = f.readline()
        for line in f:
        ...
        for (elem_read1left,elem_read2left) in zip(list(read1left),list(read2left)):
            if(abs(int(elem_read2left)-int(elem_read1left))<479)or (abs(int(elem_read2left)-int(elem_read1left)>521)):
                if int(elem_start) < int(elem_end):
    ...
    fout = open("F:/importantfilecopy/tandomrepeat/2019.121test/5results", "a")
    ...
    #     VariationList.append(Variation())
    #     VariationList[-1].SetFeature(count,incompletelymappedreads,splitmappedread,fullymappedreads,singlemappedread,readdepth,affectedread,direction,type[x])
    #     # print VariationList[-1].Result
    #     x=x+1
    # fout = open("F:/importantfilecopy/tandomrepeat/3/5x/TYPE12/results1","a")
    # for variation in VariationList:
    #     fout.write(str(variation.AbnormalRead) + " " +str(variation.IncompleteRead)+" "+str(variation.SplitRead)+" "+str(variation.FullyRead)+" "+str(variation.SingleRead)+" "+str(variation.ReadDepth)+" " +str(variation.AffectedRead)+" "+str(variation.Direction)+"\t"+str(variation.Result)+"\n")
    # q += 1